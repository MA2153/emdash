...
A [`reference` field](/reference/field-types/#reference) links an entry to entries in another collection. Its value is not part of `data`. Pass a `references` option to `getEmDashEntry` naming the fields the page renders, keyed by field slug, and each one comes back as a page of entries:
...
It reads draft visibility from the same request context as `getEmDashEntry`, so a walk started in preview keeps seeing the pending selection.
...
This creates `.emdash/types.ts` with an interface for each collection. The file also registers every collection under its slug, so the query functions infer the right interface from the name you pass:
...
const { entry: post } = await getEmDashEntry("posts", "my-post");
// post.data.title is typed
```

Each collection that has a reference field bound to a relation gets a second interface, `{Collection}References`, registered the same way. `getEmDashEntry` narrows its result to the fields the `references` option named, and each page's entries carry the target collection's interface:
...
Import an interface by name when a component or helper needs to declare the shape it accepts:
